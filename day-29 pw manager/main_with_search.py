...
def search():
    ...
    try:
        with open("pw_doc.json", "r") as data_file:
            ...
            print(search_data[website])
            result = search_data[website]
    except (FileNotFoundError, KeyError) as msg:
        messagebox.showerror(title="Error", message=f"Error: {msg}")

    else:
        messagebox.showinfo(title="Details", message=f"{website} password for {search_data[website]['email']} is"
                                                    f" {search_data[website]['password']}")
...
search_button = Button(text="Search", width=20, justify="left", command=search)
search_button.grid(column=2, row=1)

email_label = Label(text="Email/Username:")
email_label.grid(column=0, row=2)
email_input = Entry(width=34)
...
pw_button = Button(text="Generate Password", width=20, command=password_gen)
